...
///
/// Used by the Today host cards: a fixed, today-anchored trailing window over the same `repo.days`
/// the Trends tab reads, with the tab's y-padding so a hosted chart plots at the same height.
enum HostedTrendData {
    ...
    /// The metric's points for exactly the `selected` window, anchored on today. No widening onto older
    /// history: the Trends tab reports stale history rather than silently substituting it, and the host
    /// card follows the same policy (twin of the Android `TrendHostCard`). `effective` is always
    /// `selected`; it stays in the signature so a caller can label the window it received.
    static func resolve(days: [DailyMetric],
        ...
        (points(window(days, selected), value), selected)
    }
