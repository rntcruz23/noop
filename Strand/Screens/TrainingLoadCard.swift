    ...
    var body: some View {
        ...
        Group {
            if !tl.isAvailable {
                ...
                // Rows are clipped to the Trends range window; the model itself still warms up on
                // full history, so an in-window gap reads as "no readings" rather than "unavailable".
                    ...
                    ChartCard(
                        ...
                        chart: {
                            VStack(alignment: .leading, spacing: NoopMetrics.space2) {
                                legend
                                TrainingLoadChart(rows: rows, displayDomain: displayDomain)
                            }
                        },
                        footer: {
                            ChartFooter([
