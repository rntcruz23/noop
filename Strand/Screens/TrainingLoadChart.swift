...
struct TrainingLoadChart: View {
    ...
    /// The Trends range window, so the x-axis spans the selected range even when the modelled rows
    /// only cover part of it. `nil` lets Swift Charts fit the rows.
    ...
    var body: some View {
        ...
        Chart {
            ...
            // A single row draws no line segment, so mark it explicitly.
