...
class ConnectionDownReasonTest {
    ...
    /** Status 147 (0x93): the link never ESTABLISHED at all — a strap that advertises but never answers
     *  `connectGatt`. Distinct from the remote-timeout case above (an established link's supervision
     *  timing out); no origin is attached, since no bond/pairing logic ever ran to attribute it to. */
    @Test fun anEstablishmentTimeoutReadsAsEstablishTimeout() {
        assertEquals("establishTimeout", WhoopBleClient.connectionDownReason(147, null))
    }
