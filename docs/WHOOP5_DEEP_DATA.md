...
The in-app **protocol health check** (Settings → Experimental → WHOOP 5/MG → *Protocol health
check*) condenses a session into per-feature verdicts — handshake, bond, live HR, clock, frame CRCs,
command acks, R22 flag acks, offload outcome, and which type-`0x2F` layout versions the firmware
emits — in a format that is byte-identical between iOS/macOS and Android. It is read-only: it never
writes to the strap.
